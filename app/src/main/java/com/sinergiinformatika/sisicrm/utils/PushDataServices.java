...
/**
 * An Object to handle push data process to DB server
 * Created by M Fauzan Abdi on 07/02/2017.
 * base on SyncAdapter Procedure and Function
 */

public class PushDataServices implements Runnable {

    private static final String TAG = PushDataServices.class.getSimpleName();
    ...
    public PushDataServices(Context context, Handler mHandler) {
    ...
    @Override
    public void run() {
        if (Constants.DEBUG) Log.i(TAG, "push performed");
        Log.i(TAG, "token: " + User.getInstance(mContext).getToken());

        Intent intent = new Intent(Constants.INTENT_ACTION_PUSH);
        ...
        if (inboxStyle != null) {
            MiscUtil.showNotificationMessage(mContext,
                    mContext.getString(R.string.title_push_completed),
                    null,
                    inboxStyle);
        }
        ...
        if (Constants.DEBUG) Log.i(TAG, "push finished");
    }
    ...
    private List<Order> getPendingOrders() {
        ...
        return orderTable.getPendingOrders(LIMIT_NUMBER);
    }
    ...
    private void pushPendingStores() {
        ...
        List<Store> stores = storeTable.getPendingStores(LIMIT_NUMBER);
    ...
    private void pushPendingSurvey() {
        ...
        List<Survey> surveyList = surveyTable.getPendingSurveyList(LIMIT_NUMBER);
    ...
    private void pushPendingSurveyPhotos() {
        final SurveyTable surveyTable = new SurveyTable(mContext);
        String token = User.getInstance(mContext).getToken();
        List<Survey> surveyList = surveyTable.getPendingSurveyPhotos(LIMIT_NUMBER);

        for (Survey survey : surveyList) {
            if (survey.getImages() != null && !survey.getImages().isEmpty()) {
                PushDataServices.SurveyPhotoResponseHandler responseHandler =
                        new PushDataServices.SurveyPhotoResponseHandler(mContext, survey.getId());
    ...
    public HttpClient createNewHttpClient() {
        try {
            ...
            PushDataServices.MySSLSocketFactory sf = new PushDataServices.MySSLSocketFactory(trustStore);
